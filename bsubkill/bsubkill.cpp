...
const bool BSUBKILL_DEBUG = false;
...
bool verifyUserMapping(std::string fpath, std::string execName)
{
    ...
    if (userpsw == NULL) {
        std::cout << "Failed to retrieve current user" << std::endl;
        return false;
    }
...
int changeUser(const char * execUser)
{
    ...
    if (verifyUserMapping(mappingfile, execUser)) {
        ...
        std::cout << "Failed to kill the job for user " << execUser << std::endl;
        return -1;
    }
    if (BSUBKILL_DEBUG)
...
{
    std::string usage("Usage: bsubkill job-id");
    ...
    std::string bjobscmd = "bjobs -noheader -o \"user\" ";
    bjobscmd.append(argv[1]);

    std::ostringstream oss;
    int exit_code = runcmd(bjobscmd, oss);
    if (exit_code == 0) {
        ...
        if (changeUser(user.c_str()) != 0) {
            return -1;
        }

        std::string bkillcmd = "bkill ";
        bkillcmd.append(argv[1]);
        exit_code = runcmd(bkillcmd);
    }
